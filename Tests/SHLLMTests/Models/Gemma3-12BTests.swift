...
struct Gemma3_12BTests {
    ...
    func canStreamResult() async throws {
        ...
        guard let llm = try gemma3_12B(input: input) else { return }
    ...
    func canExtractTextFromImageData() async throws {
        ...
        guard let llm = try gemma3_12B(image: data) else { return }
    ...
    func canExtractTextFromImageURL() async throws {
        ...
        guard let llm = try gemma3_12B(image: url) else { return }
...
private extension Gemma3_12BTests {
    func gemma3_12B(
        input: UserInput
    ) throws -> LLM<Gemma3>? {
        try loadModel(
            directory: LLM.gemma3_12B,
            input: input,

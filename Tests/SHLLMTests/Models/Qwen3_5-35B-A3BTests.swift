...
struct Qwen3_5_35B_A3BTests {
    ...
    func canStreamResult() async throws {
        ...
        guard let llm = try qwen3_5MoE(input) else { return }
    ...
    func canCompleteMultiToolWorkflowAndEmail() async throws {
        ...
        var input = UserInput(chat: chat)
        guard let llm = try qwen3_5MoE(input, tools: [
        ...
        guard let llm2 = try qwen3_5MoE(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        guard let llm3 = try qwen3_5MoE(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        guard let llm4 = try qwen3_5MoE(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        guard let llm5 = try qwen3_5MoE(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
    ...
    func canExtractTextFromImageData() async throws {
        ...
        guard let llm = try qwen3_5MoE(image: data) else { return }
    ...
    func canExtractTextFromImageURL() async throws {
        ...
        guard let llm = try qwen3_5MoE(image: url) else { return }
...
private func qwen3_5MoE(
    _ input: UserInput,
    tools: [any ToolProtocol] = []
) throws -> LLM<Qwen35MoE>? {
    try loadModel(
        directory: LLM<Qwen35MoE>.qwen3_5MoE__35B_A3B,
        input: input,
        tools: tools,
        responseParser: LLM<Qwen35MoE>.qwen3_5MoEParser(for: input)
    )
}

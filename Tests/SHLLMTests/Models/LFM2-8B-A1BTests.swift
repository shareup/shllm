...
struct LFM2_8B_A1BTests {
    ...
    func canUseStockToolAndRespond() async throws {
        ...
        guard let llm1 = try lfm2_8B_A1B(
            input,
            tools: [stockTool]
        ) else { return }
    ...
    func canCompleteMultiToolWorkflowAndEmail() async throws {
        let chat: [Chat.Message] = [
            .system("""
                ...
                Each tool must be called in order, and the output of one tool \
                may be needed as input for the next tool.
                """
            ),
            .user(
        ...
        // web_search
        var input = UserInput(chat: chat)
        guard let llm = try lfm2_8B_A1B(input, tools: [
        ...
        // fetch_web_page
        guard let llm2 = try lfm2_8B_A1B(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        guard let llm3 = try lfm2_8B_A1B(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        // send_email
        guard let llm4 = try lfm2_8B_A1B(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }
        ...
        // assistant response
        guard let llm5 = try lfm2_8B_A1B(input, tools: [
            webSearchTool, fetchPageTool, findEmailTool, sendEmailTool,
        ]) else { return }

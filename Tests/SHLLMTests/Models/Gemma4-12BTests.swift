...
struct Gemma4_12BTests {
    ...
    func canStreamResult() async throws {
        ...
        guard let llm = try gemma4_12B(input: input) else { return }
    ...
    func canExtractTextFromImageData() async throws {
        ...
        guard let llm = try gemma4_12B(image: data) else { return }
    ...
    func canExtractTextFromImageURL() async throws {
        ...
        guard let llm = try gemma4_12B(image: url) else { return }
    ...
    func canFetchTheWeather() async throws {
        ...
        guard let llm = try gemma4_12B(input, tools: [weatherTool]) else { return }
    ...
    func canUseStockToolAndRespond() async throws {
        ...
        guard let llm1 = try gemma4_12B(input, tools: [stockTool]) else { return }
        ...
        guard let llm2 = try gemma4_12B(input, tools: [stockTool]) else { return }
    ...
    func canUseToolsWithNonStringArgumentsAndRespond() async throws {
        ...
        guard let llm1 = try gemma4_12B(input, tools: [mailReadTool]) else { return }
        ...
        guard let llm2 = try gemma4_12B(input, tools: [mailReadTool]) else { return }
...
private extension Gemma4_12BTests {
    func gemma4_12B(
        _ input: UserInput,
        tools: [any ToolProtocol] = []
    ) throws -> LLM<Gemma4Unified>? {
        try loadModel(
            directory: LLM.gemma4_12B,
            input: input,
            tools: tools,
            ...
            responseParser: LLM<Gemma4Unified>.gemma4Parser
        )
    }

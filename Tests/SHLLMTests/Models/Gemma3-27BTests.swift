...
struct Gemma3_27BTests {
    ...
    func canStreamResult() async throws {
        ...
        guard let llm = try gemma3_27B(input: input) else { return }
    ...
    func canExtractTextFromImageData() async throws {
        ...
        guard let llm = try gemma3_27B(image: data) else { return }
    ...
    func canExtractTextFromImageURL() async throws {
        ...
        guard let llm = try gemma3_27B(image: url) else { return }
...
private extension Gemma3_27BTests {
    func gemma3_27B(
        input: UserInput
    ) throws -> LLM<Gemma3>? {
        try loadModel(
            directory: LLM.gemma3_27B,
            input: input,
